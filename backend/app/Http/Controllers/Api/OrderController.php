...
class OrderController extends Controller
{
    ...
    {
        $query = Order::with(['items.menuItem', 'payments', 'creator'])->orderByDesc('created_at');
        ...
        if ($request->boolean('all', false) || $request->filled('date')) {
            return $query->get();
        }
    ...
    public function store(Request $request, BusinessHours $businessHours)
    {
        try {
            $data = $request->validate([
                ...
                'items.*.side_choice' => 'nullable|string|max:100',
            ...
            return DB::transaction(function () use ($data, $request) {
                ...
                // Total quantity requested per menu_item_id across all item lines (for stock checking)
                $stockDemand = collect($data['items'])
                    ->groupBy('menu_item_id')
                    ->map(fn ($items) => $items->sum('quantity'));

                $menuItems = MenuItem::whereIn('id', $stockDemand->keys())
                    ->lockForUpdate()
                    ->get()
                    ->keyBy('id');

                // Validate total stock demand first
                foreach ($stockDemand as $menuItemId => $totalRequestedQty) {
                    ...
                    if ($menuItem->stock !== null && $menuItem->stock < $totalRequestedQty) {
                ...
                // Deduct stock once per menu item
                foreach ($stockDemand as $menuItemId => $totalRequestedQty) {
                    $menuItem = $menuItems->get($menuItemId);
                    if ($menuItem->stock !== null) {
                        $menuItem->stock -= $totalRequestedQty;
                        if ($menuItem->stock <= 0) {
                            $menuItem->stock = 0;
                        ...
                        InventoryAdjustment::create([
                            ...
                            'quantity_change' => -$totalRequestedQty,
                            'reason' => 'order',
                ...
                // Process individual item lines (preserving distinct side_choice selections)
                foreach ($data['items'] as $itemInput) {
                    $menuItem = $menuItems->get($itemInput['menu_item_id']);
                    $price = $menuItem->price;
                    $quantity = (int) $itemInput['quantity'];
                    $lineTotal = $price * $quantity;
                    $sideChoice = ! empty($itemInput['side_choice']) ? trim($itemInput['side_choice']) : null;
                ...
                DB::afterCommit(function () use ($order, $changedMenuItems) {
                    try {
                        ...
                        Log::warning('Order broadcast failed', ['error' => $e->getMessage()]);
                    }
                    try {
                        $changedMenuItems->each(fn (MenuItem $item) => $this->broadcastMenuItemChange($item));
                    } catch (\Throwable $e) {
                        Log::warning('MenuItem broadcast failed', ['error' => $e->getMessage()]);
                    }
                });
            ...
            Log::error('Order creation 500 error: '.$e->getMessage(), [
                'exception' => $e,
                'request' => $request->all(),
            ]);
    ...
    public function updateKitchenStatus(Request $request, Order $order)
    {
        ...
        $newKitchenStatus = $data['kitchen_status'];
        $order->fill([
        ...
        // Marking kitchen_status as ready or served automatically completes the order
        if (in_array($newKitchenStatus, ['ready', 'served'], true)) {
    ...
    public function destroy(Request $request, Order $order)
    {
        $user = $request->user();
        $isAdmin = $user && (
            $user->hasAnyRole(['admin', 'super_admin', 'super admin']) ||
            $user->role === 'admin' ||
            $user->role === 'super_admin' ||
            $user->email === 'admin@afc.com.ng'
        );

        if (! $isAdmin && $order->status !== 'pending') {
            return response()->json([
                'message' => 'Only super admin or admin can delete completed or non-pending orders.',
            ], 403);
        }
    ...
    public function updateStatus(Request $request, Order $order)
    {
        ...
        // Completed or ready/served orders cannot be cancelled or modified by managers
        $isLocked = $order->status === 'completed' || in_array($order->kitchen_status, ['ready', 'served'], true);
        if ($isLocked && ! $request->user()?->hasRole('admin')) {
            return response()->json([
                'message' => 'Completed or ready orders cannot be modified by managers. Only admins can delete orders.',
            ], 403);
        }
        ...
        return $order->fresh(['items.menuItem', 'payments', 'creator']);
    }

    public function managerOrders(Request $request)
    {
        return Order::with(['items.menuItem', 'payments', 'creator'])
    ...
    {
        // Exclude cancelled orders from stats
